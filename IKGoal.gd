...
func ik():
	...
	var full_length = 0
	for bone in bones:
		full_length += bone.length
		
	if bones[0].global_position.distance_to(global_position) > full_length:
		for i in range(len(bones)):
			var ang = bones[i].global_position.angle_to_point(global_position) + deg2rad(180)
		...
		while limit > 0 and bones[-1].global_position.distance_to(global_position) > error:
			...
			for i in range(len(pos)-2, -1, -1):
				pos[i] = pos[i+1] + (pos[i] - pos[i+1]).normalized() * bones[i].length
			
			pos[0] = bones[0].global_position
			for i in range(1, len(pos)-1):
				pos[i] = pos[i-1] + (pos[i] - pos[i-1]).normalized() * bones[i-1].length
			
			for i in range(len(pos)-1):
				bones[i].global_rotation = bones[i].global_position.angle_to_point(pos[i+1]) + deg2rad(180)
